...
public class ExcelExport {
	...
	// 表头字体
	private String titleFontType = "新宋体";
	// 表头背景色
	...
	// 正文字体
	private String contentFontType = "华文楷体";
	// 正文字号
	private short contentFontSize = 12;
	// Float类型数据小数位
	//private String floatDecimal = "0";
	// Double类型数据小数位
	//private String doubleDecimal = "0";
	// 设置列的公式
	...
	public void wirteExcel(String titleColumn[], String titleName[], int titleSize[], List<?> dataList) {
		...
		try {
			...
			titleStyle = (HSSFCellStyle) setFontAndBorderToTitle(titleStyle, titleFontType, (short) titleFontSize);
			...
			if (dataList != null && dataList.size() > 0) {
				...
				dataStyle = (HSSFCellStyle) setFontAndBorderData(dataStyle, contentFontType, (short) contentFontSize);
					...
					for (int rowIndex = 1; rowIndex <= dataList.size(); rowIndex++) {
						...
						for (int columnIndex = 0; columnIndex < titleColumn.length; columnIndex++) {
							...
							if (!"".equals(title)) { // 字段不为空
								...
								cell.setCellStyle(dataStyle); //加入样式
								...
								if (data != null && !"".equals(data)) {
									if ("int".equals(returnType)) {
										...
										//doubleDecimalFormat.format(Double.parseDouble(data))
									} else {
	 ...
	 */
	@SuppressWarnings({"unchecked", "rawtypes"})
	public void wirteExcelViaJson(String titleColumn[], String titleName[], int titleSize[], List<JSONObject> dataList) {
		// 添加Worksheet（不添加sheet时生成的xls文件打开时会报错)
		Sheet sheet = workbook.createSheet(this.sheetName);
		//设置宽度
		sheet.setDefaultRowHeight((short) 420);
		...
		try {
			...
			if (dataList != null && dataList.size() > 0) {
				...
				if (titleColumn.length > 0) {
					for (int rowIndex = 1; rowIndex <= dataList.size(); rowIndex++) {
						JSONObject obj = dataList.get(rowIndex - 1); // 获得该对象
						Row dataRow = workbook.getSheet(sheetName).createRow(rowIndex);

						//更据titleColumn获取cell的值
						for (int columnIndex = 0; columnIndex < titleColumn.length; columnIndex++) {
							String cellValue = obj.getString(titleColumn[columnIndex]);
							Cell cell = dataRow.createCell(columnIndex);
							cell.setCellStyle(dataStyle); //加入样式
	 ...
	 */
	@SuppressWarnings({"unchecked", "rawtypes"})
	public void wirteExcelLocal(String filePath, String titleColumn[], String titleName[], int titleSize[], List<?> dataList) {
		// 添加Worksheet（不添加sheet时生成的xls文件打开时会报错)
		Sheet sheet = workbook.createSheet(this.sheetName);
		//设置宽度
		sheet.setDefaultRowHeight((short) 420);
	 ...
	 * @return
	 */
	public CellStyle setFontAndBorderToTitle(CellStyle style, String fontName, short size) {
		HSSFFont font = workbook.createFont();
		font.setFontHeightInPoints(size);
		font.setFontName(fontName); //字体
		font.setBoldweight(HSSFFont.BOLDWEIGHT_BOLD); //加粗
		style.setFont(font);  //设置对应的字体
	 ...
	 * @return
	 */
	public CellStyle setFontAndBorderData(CellStyle style, String fontName, short size) {
		HSSFFont font = workbook.createFont();
		font.setFontHeightInPoints(size);
		font.setFontName(fontName); //字体
		style.setFont(font);  //设置对应的字体
